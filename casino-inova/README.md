...
- `app/` — esqueleto do app mobile (Expo + React Native + TypeScript), com o sistema de design (cores, tipografia), navegação entre as 5 áreas principais (Lobby, Torneios, Loja, Amigos, Perfil), tutorial "como jogar" por jogo (`src/data/tutorials.ts` + `TutorialModal`) escrito em linguagem simples para quem nunca jogou, e uma tela de mesa genérica para os 7 jogos que ainda não têm motor. **Slots já é jogável de verdade** (`SlotsScreen`): fala com a API do servidor, mostra o RTP divulgado, gira, debita/credita fichas de verdade. Os outros 7 ainda usam dados mockados.
- `server/` — esqueleto do backend (NestJS + TypeScript) com os módulos de usuário, carteira de fichas (ledger append-only), loja e o **motor de slots** — o primeiro jogo com regras de verdade: RTP calculado por fórmula exata (não chutado, ver `slots.engine.ts` e `npm run verify:rtp`), aposta debitada e prêmio creditado no mesmo ledger da carteira. Tudo funcional em memória, testável com `curl`. Ver `server/README.md` para o que falta pra virar a Fase 0 de verdade (banco real, autenticação, validação de recibo).
