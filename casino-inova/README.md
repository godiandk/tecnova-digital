...
- `app/` — esqueleto do app mobile (Expo + React Native + TypeScript), com o sistema de design (cores, tipografia), navegação entre as 5 áreas principais (Lobby, Torneios, Loja, Amigos, Perfil), tutorial "como jogar" por jogo (`src/data/tutorials.ts` + `TutorialModal`) escrito em linguagem simples para quem nunca jogou, e uma tela de mesa genérica para os 2 jogos que ainda não têm motor (Dominó, Pôquer). **Fase 1, Fase 2 e Truco (Fase 3) já são jogáveis de verdade** — Slots, Roleta, Blackjack, Bacará, Banca Francesa e Truco falam com a API do servidor, mostram o RTP divulgado quando ele existe, apostam, debitam/creditam fichas de verdade. Truco é contra bot, ainda sem multiplayer de verdade (ver nota abaixo). Perfil tem um campo de resgatar cupom. As **38 imagens do briefing já estão integradas** (`app/assets/images/`, otimizadas de 128MB pra 55MB): fundo do Lobby, mesa de cada jogo como pano de fundo real, e o retrato do crupiê ao lado do título nas mesas com dealer — a Banca Francesa mostra os 3 personagens da equipe (banqueiro, tirador, apontador).
- `server/` — esqueleto do backend (NestJS + TypeScript) com os módulos de usuário, carteira de fichas (ledger append-only), loja e 5 motores de jogo de verdade — a Fase 1 inteira mais a Fase 2 inteira: **slots** (RTP por fórmula exata, `npm run verify:rtp`), **roleta europeia** (RTP fixo de 36/37 ≈ 97,30% por propriedade matemática), **blackjack** (mão com estado — apostar/pedir carta/parar —, blackjack paga 3:2, `npm run verify:blackjack` simula uma estratégia de referência já que RTP de blackjack depende de como se joga), **bacará** (regras reais de compra da 3ª carta do Punto Banco, banca paga com 5% de comissão, `npm run verify:bacara`) e **banca francesa** (3 dados, apostas simultâneas em vários números, RTP fixo de 199/216 ≈ 92,13% — o mesmo do "Chuck-a-Luck" internacional, `npm run verify:banca-francesa`). Também tem **cupons** (admin cria, jogador resgata por código), **papéis/permissões** (admin e moderador — moderador nunca pode banir nem criar outro moderador; a matriz completa de quem pode fazer o quê está em `server/README.md`) e **truco contra bot** (regras reais — manilha, força de carta, desempate de mão, pedir truco — mas multiplayer de verdade com outro jogador ainda exige sala + WebSocket, que não existe neste esqueleto). Tudo funcional em memória, testável com `curl`. Ver `server/README.md` para o que falta pra virar a Fase 0 de verdade (banco real, autenticação, validação de recibo).
