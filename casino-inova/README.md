...
- `app/` — esqueleto do app mobile (Expo + React Native + TypeScript), com o sistema de design (cores, tipografia), navegação entre as 5 áreas principais (Lobby, Torneios, Loja, Amigos, Perfil), tutorial "como jogar" por jogo (`src/data/tutorials.ts` + `TutorialModal`) escrito em linguagem simples para quem nunca jogou, e uma tela de mesa genérica para os 5 jogos que ainda não têm motor. **A Fase 1 inteira já é jogável de verdade** — Slots, Roleta e Blackjack (`SlotsScreen`, `RouletteScreen`, `BlackjackScreen`) falam com a API do servidor, mostram o RTP divulgado quando ele existe, apostam, debitam/creditam fichas de verdade. Os outros 5 jogos (Fases 2-4) ainda usam dados mockados.
- `server/` — esqueleto do backend (NestJS + TypeScript) com os módulos de usuário, carteira de fichas (ledger append-only), loja e os **3 motores de jogo da Fase 1**: **slots** (RTP por fórmula exata, `npm run verify:rtp`), **roleta europeia** (RTP fixo de 36/37 ≈ 97,30% por propriedade matemática) e **blackjack** (mão com estado — apostar/pedir carta/parar —, blackjack paga 3:2, `npm run verify:blackjack` simula uma estratégia de referência já que RTP de blackjack depende de como se joga). Tudo funcional em memória, testável com `curl`. Ver `server/README.md` para o que falta pra virar a Fase 0 de verdade (banco real, autenticação, validação de recibo).
