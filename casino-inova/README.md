...
- `app/` — esqueleto do app mobile (Expo + React Native + TypeScript), com o sistema de design (cores, tipografia), navegação entre as 5 áreas principais (Lobby, Torneios, Loja, Amigos, Perfil), tutorial "como jogar" por jogo (`src/data/tutorials.ts` + `TutorialModal`) escrito em linguagem simples para quem nunca jogou, e uma tela de mesa genérica para os 4 jogos que ainda não têm motor. **A Fase 1 inteira, mais Bacará da Fase 2, já são jogáveis de verdade** — Slots, Roleta, Blackjack e Bacará (`SlotsScreen`, `RouletteScreen`, `BlackjackScreen`, `BaccaratScreen`) falam com a API do servidor, mostram o RTP ou multiplicador divulgado, apostam, debitam/creditam fichas de verdade. Os outros 4 jogos (Banca Francesa, Truco, Dominó, Pôquer) ainda usam dados mockados.
- `server/` — esqueleto do backend (NestJS + TypeScript) com os módulos de usuário, carteira de fichas (ledger append-only), loja e 4 motores de jogo de verdade: **slots** (RTP por fórmula exata, `npm run verify:rtp`), **roleta europeia** (RTP fixo de 36/37 ≈ 97,30% por propriedade matemática), **blackjack** (mão com estado — apostar/pedir carta/parar —, blackjack paga 3:2, `npm run verify:blackjack` simula uma estratégia de referência já que RTP de blackjack depende de como se joga) e **bacará** (regras reais de compra da 3ª carta do Punto Banco, banca paga com 5% de comissão, `npm run verify:bacara` mede o RTP real de cada aposta por simulação, já que bacará não tem estratégia possível). Tudo funcional em memória, testável com `curl`. Ver `server/README.md` para o que falta pra virar a Fase 0 de verdade (banco real, autenticação, validação de recibo).
