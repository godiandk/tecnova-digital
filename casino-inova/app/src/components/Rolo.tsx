...
interface RoloProps {
  /** Os três símbolos que o servidor sorteou pra esta coluna, de cima pra baixo. */
  resultado: string[] | null;
  /** Ligado enquanto o giro está em andamento. */
  girando: boolean;
  /** 0, 1 ou 2 — define quanto essa coluna demora a mais pra parar. */
  coluna: number;
  altura: number;
  largura: number;
}
...
/** Cada coluna para 260 ms depois da anterior — é o que dá o "tec, tec, tec". */
 ...
 * o fim da tira, onde estão os três símbolos que o servidor sorteou, e para com um
 ...
 */
export function Rolo({ resultado, girando, coluna, altura, largura }: RoloProps) {
   ...
   */
  const tira = useMemo(
    () => [...ruido, ...(resultado ?? sortearRuido(3))],
    [ruido, resultado],
  );
  ...
  useEffect(() => {
    if (girando) {
      ...
      deslocamento.value = withRepeat(
        withTiming(-(3 * altura), { duration: VOLTA_EM_MS, easing: Easing.linear }),
        -1,
        false,
      );
      return;
  ...
  }, [girando, resultado, altura, coluna, fim, deslocamento]);

  const animado = useAnimatedStyle(() => ({ transform: [{ translateY: deslocamento.value }] }));

  return (
    <View style={[styles.janela, { width: largura, height: altura * 3 }]}>
...
const styles = StyleSheet.create({
  // `overflow: hidden` é o que transforma a tira num rolo: só três símbolos aparecem.
